...
async function fixOrders() {
  try {
    ...
    // Find orders for Shashwat Yadav
    const orders = await Order.find({ 
      'customerInfo.name': { $regex: 'shashwat yadav', $options: 'i' } 
    }).sort({ createdAt: -1 }).limit(2);

    if (orders.length === 0) {
      console.log('No orders found for Shashwat Yadav!');
      process.exit(1);
    }

    console.log(`Found ${orders.length} orders for Shashwat Yadav. Updating...`);
