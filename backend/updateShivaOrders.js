...
async function fixOrders() {
  try {
    ...
    // Find orders for Shiva
    const orders = await Order.find({ 
      'customerInfo.name': { $regex: 'shiva', $options: 'i' } 
    }).sort({ createdAt: -1 }).limit(10); // Check recent orders

    if (orders.length === 0) {
      console.log('No orders found for Shiva!');
      process.exit(1);
    }

    console.log(`Found ${orders.length} orders for Shiva. Updating...`);
